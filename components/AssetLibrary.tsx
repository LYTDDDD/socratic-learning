          ...
          {reviewState && (
            <div className="border-t border-line pt-4">
              <div className="mb-3 flex items-center justify-between">
                <dt className="text-xs font-semibold text-ink/70">AI 评估（Review Mode）</dt>
                <button
              ...
              {reviewState.phase === "loading_questions" && (
              ...
              {reviewState.phase === "answering" && (
                <div className="space-y-3">
                  {reviewState.questions.map((q, i) => (
                    <div key={i} className="rounded-lg border border-line bg-paper/60 p-3">
                      ...
                      <textarea
                        ...
                        onChange={(e) => updateReviewAnswer(i, e.target.value)}
                        placeholder="写下你的回答..."
                        rows={3}
                        value={reviewState.answers[i]}
                      />
                    </div>
                  ))}
                  <button
                    className="rounded-lg bg-moss px-4 py-2 text-sm font-semibold text-white transition hover:bg-moss/90"
                    disabled={reviewState.answers.some((a) => !a.trim())}
                    onClick={submitReviewAnswers}
                    type="button"
                  >
                    提交评估
                  </button>
              ...
              {reviewState.phase === "loading_feedback" && (
                <div className="flex items-center gap-2 rounded-lg border border-moss/30 bg-moss/5 px-4 py-6 text-sm text-ink/60">
                  <svg className="h-4 w-4 animate-spin text-moss" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    <path className="opacity-75" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" fill="currentColor" />
              ...
              {reviewState.phase === "result" && (
                <div className="space-y-3">
                  {reviewState.feedback.map((f, i) => (
                  ...
                  <div className="rounded-lg border border-line bg-paper/60 p-3">
                    ...
                    <dd className="text-sm text-ink">{reviewState.overallAssessment || "—"}</dd>
                  </div>
                  <p className="text-[10px] text-ink/40">
                    {reviewState.recordSaved ? "复习记录已保存。" : "复习记录保存失败，请稍后重试。"}
                  </p>
                  {reviewState.maturitySuggestion && (
                    <div className="rounded-lg border border-moss/30 bg-moss/5 p-3">
                      ...
                      <dd className="text-sm text-ink">
                        从 <span className="font-medium text-rust">{reviewState.maturitySuggestion.current}</span> 升级到{" "}
                        <span className="font-medium text-moss">{reviewState.maturitySuggestion.suggested}</span> — {reviewState.maturitySuggestion.reason}
                      </dd>
              ...
              {reviewState.phase === "error" && (
                <div className="flex items-start gap-2 rounded-lg border border-rust/20 bg-rust/5 p-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-ink/50">{reviewState.message}</p>
                    <button
