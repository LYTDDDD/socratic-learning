    ...
    <div className={className}>
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 truncate text-xs text-ink-muted">{reviewFlow.asset.title || reviewFlow.asset.asset_id}</p>
        </div>
        <button
          className="rounded px-2 py-1 text-xs font-medium text-ink-muted/70 transition hover:bg-surface-2 hover:text-ink"
          onClick={onExit}
          type="button"
        >
          {exitLabel}
        ...
        <div className="space-y-3">
          {reviewFlow.questions.map((question, index) => (
            <div key={index} className="rounded-lg border border-line bg-surface-1 p-3">
              ...
              <textarea
                className="w-full rounded-md border border-line bg-surface-1 px-2.5 py-1.5 text-sm text-ink outline-none transition focus:border-blue focus:ring-1 focus:ring-blue/20"
        ...
        <div className="space-y-3">
          {reviewFlow.feedback.map((feedback, index) => (
            <div key={index} className={`rounded-lg border p-3 ${feedbackSurfaceColor(feedback.evaluation)}`}>
              <div className="mb-1 flex items-center gap-2">
                ...
                <span className="text-[10px] text-ink-muted/70">问题 {index + 1}</span>
              </div>
              <p className="mb-1 text-xs font-medium text-ink-muted">{feedback.question}</p>
              <p className="mb-1 text-xs text-ink-muted italic">你的回答：{feedback.answer || "（未回答）"}</p>
              <p className="text-xs text-ink">{feedback.comment}</p>
            </div>
          ))}
          <div className="rounded-lg border border-line bg-surface-1 p-3">
            <dt className="mb-1 text-xs font-semibold text-ink-muted">整体评估</dt>
          ...
          {reviewFlow.maturitySuggestion && (
            <div className="rounded-lg border border-blue/30 bg-blue/5 p-3">
              <dt className="mb-1 flex items-center gap-1 text-xs font-semibold text-blue">
              ...
              <dd className="text-sm text-ink">
                从 <span className="font-medium text-amber">{reviewFlow.maturitySuggestion.current}</span> 升级到{" "}
                <span className="font-medium text-blue">{reviewFlow.maturitySuggestion.suggested}</span> — {reviewFlow.maturitySuggestion.reason}
              </dd>
              <p className="mt-1 text-[10px] text-ink-muted/70">这是建议，不会自动修改资产。</p>
            </div>
          )}
          {resultActions}
        </div>
      ...
      {reviewFlow.phase === "error" && (
        <div className="flex items-start gap-2 rounded-lg border border-amber/20 bg-amber/5 p-3">
          ...
          <div>
            <p className="text-sm text-amber">Review 出错</p>
            <p className="mt-0.5 text-xs text-ink-muted">{reviewFlow.message}</p>
            <button
              className="mt-2 rounded border border-amber/30 px-3 py-1 text-xs font-medium text-amber transition hover:bg-amber/10"
              onClick={onRetry}
              type="button"
            >
              重试
...
function LoadingMessage({ children }: { children: ReactNode }) {
  return (
    <div className="flex items-center gap-2 rounded-lg border border-blue/30 bg-blue/5 px-4 py-6 text-sm text-ink-muted">
      <svg className="h-4 w-4 animate-spin text-blue" fill="none" viewBox="0 0 24 24">
